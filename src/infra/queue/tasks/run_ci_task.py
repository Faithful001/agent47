...
def run_ci_task(build_id: str, repo_id: str):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logger.info("Building dynamic container image with Nixpacks for %s", repo.full_name)
            ...
            try:
                # Nixpacks will auto-detect the tech stack and create a Docker image
                # MAKE SURE NIXPACKS IS INSTALLED ON THE HOST: curl -sSL https://nixpacks.com/install.sh | bash
                subprocess.run(
                    ["nixpacks", "build", build_dir, "--name", image_name],
                    check=True,
                    capture_output=True,
                )
            except FileNotFoundError:
                logger.error("Nixpacks CLI is not installed on the system.")
                return
            except subprocess.CalledProcessError as e:
                logger.error("Nixpacks build failed: %s", e.stderr.decode('utf-8'))
                return

            # The environment is built! Now we execute the user's specific test commands in it
            # Nixpacks places the built code in /app by default
            script = f"""
            cd /app
            {install_cmd}
            {build_cmd}
            ...
            try:
                logger.info("Executing custom CI test commands inside Nixpacks container")
            ...
            repo_url_clean = f"https://github.com/{repo.full_name}.git"
