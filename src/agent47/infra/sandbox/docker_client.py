...
class Sandbox:
    ...
    def execute_command(self, command: str, workdir: str | None = "/workspace") -> str:
        """Executes a command inside the container and returns the output."""
        if not self.container:
            raise RuntimeError("Sandbox is not running.")
        
        exec_kwargs = {"workdir": workdir} if workdir else {}
    ...
    def copy_repo_to_container(self, local_dir: str, container_dir: str):
        ...
        def exclude_filter(tarinfo):
            skip_dirs = ("node_modules", ".venv", "__pycache__", ".next", "dist", ".cache", ".git")
            parts = os.path.normpath(tarinfo.name).split(os.sep)
        ...
        tar_stream = io.BytesIO()
        with tarfile.open(fileobj=tar_stream, mode="w") as tar:
            tar.add(local_dir, arcname=".", filter=exclude_filter)

        tar_stream.seek(0)
        self.execute_command(f"mkdir -p {container_dir}", workdir=None)
        ...
        with tarfile.open(fileobj=tar_stream, mode="r") as tar:
            ...
            for member in tar.getmembers():
                ...
                if member.name:  # skip empty names
                    ...
                    # Skip dependency/build directories. They contain symlinks that break on Windows
                    skip_dirs = ("node_modules/", ".venv/", "__pycache__/", ".next/", "dist/", ".cache/")
                    if any(member.name.startswith(d) or f"/{d}" in member.name for d in skip_dirs):
                        continue
                    ...
                    except (KeyError, OSError) as extract_err:
                        # Gracefully skip broken symlinks or permission issues
                        logger.warning("Skipping unextractable member %s: %s", member.name, extract_err)
