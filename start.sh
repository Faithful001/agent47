...
# Start Celery worker in background (single process / solo pool for low memory overhead)
celery -A agent47.infra.queue worker --loglevel=info --concurrency=1 --pool=solo --without-mingle --without-gossip &
